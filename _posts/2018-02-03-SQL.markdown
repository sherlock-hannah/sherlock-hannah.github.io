...
### GROUP BY
`GROUP BY listname`group by list,arrange rows by group

Grouping Operators
- `GROUP BY year ASC WITH ROLLUP;` ROLLUP the query produces an extra super-aggregate summary row
- `GROUPING()`
...
```
avg(columnname) over (order by columnname1 row 2 preceding)
```
...
- outer apply conceptually similae to left join
```
...
- 10.install problems:maybe compatibility errors
